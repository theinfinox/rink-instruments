...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function TaxonomyBuilder({ taxonomy, availableColumns = [], tabs = [], onChange }: {
  ...
  const addStandaloneValues = (categoryId: string) => {
    // Ungrouped values stored under empty string key ""
  ...
  const updateJoinSource = (categoryId: string, field: string, value: string | boolean | number) => {
    onChange(currentTaxonomy.map(c => {
      if (c.id !== categoryId) return c;
      return {
        ...c,
        joinSource: { ...(c.joinSource || { gid: 0, foreignKey: '', groupByColumn: '', displayColumn: '' }), [field]: value }
      };
    }));
  };
        ...
        {currentTaxonomy.map(category => (
          <div key={category.id} className="border border-indigo-100 bg-white p-4 rounded relative shadow-sm">
            <button
              ...
              className="absolute top-4 right-4 text-red-500 hover:text-red-700 text-xs font-medium"
            >
              Remove Category
            </button>
            ...
            <h4 className="font-bold text-indigo-900 mb-2 flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              {availableColumns.length > 0 ? (
                ...
                >
                  ...
                  {availableColumns.filter(c => c !== category.id).map(col => (
                    <option key={col} value={col} disabled={currentTaxonomy.some(c => c.id === col)}>{col}</option>
                  ))}
                </select>
              ) : (
            ...
            {category.linkType === 'join' && (
              <div className="mb-3 p-3 bg-emerald-50 border border-emerald-200 rounded space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" placeholder="Column in lookup tab (e.g. provider_key)"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" placeholder="Column whose values become group names (e.g. institution_type)"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" placeholder="Column shown to user (e.g. institution_name)"
            ...
            {/* Groups & Values */}
            {/* For join: manual override groups; For direct: normal groups */}
                    ...
                    <div key={group} className="bg-gray-50 p-3 rounded border border-gray-200">
                      {group !== '' && (
                        <div className="flex items-center justify-between mb-2">
                          <button onClick={() => renameGroup(category.id, group)}
                            className="text-sm font-semibold text-gray-800 hover:text-indigo-600 transition-colors"
                            title="Click to rename group">{group}</button>
                          <button onClick={() => removeGroup(category.id, group)}
            ...
            <div className="flex gap-2 flex-wrap">
              {!Object.keys(category.groups).includes('') && category.linkType !== 'join' && (
                <button onClick={() => addStandaloneValues(category.id)}
                  className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-2 py-1 rounded font-medium border border-gray-200 transition-colors">
                  + Add Values (no group)
                </button>
              )}
              <button onClick={() => addGroup(category.id)}
                className="text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-2 py-1 rounded font-medium border border-indigo-200 transition-colors">
