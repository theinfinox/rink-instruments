...
function PartnerCard({ 
  ...
  const wrapperClass = `bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-200/90 shadow-sm hover:shadow-md hover:border-blue-300 transition-all duration-200 flex-col justify-between group ${
    partner.slug ? 'hover:-translate-y-1 cursor-pointer' : ''
  } ${hiddenOnMobile ? 'hidden sm:flex' : 'flex'}`;

  return partner.slug ? (
    <Link href={`/institutions/${partner.slug}?from=partnerships`} className={wrapperClass}>
      {innerContent}
    </Link>
  ) : (
