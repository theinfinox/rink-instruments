...
export default async function ServiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div id="mobile-sticky-cta" className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white/95 backdrop-blur-md border-t border-slate-200 px-4 pt-3 pb-[calc(env(safe-area-inset-bottom,16px)+8px)] shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="flex gap-3">
          <NativeShareButton 
            ...
            url={canonicalUrl} 
            variant="outline"
            className={formattedBookingUrl ? "flex-none w-[35%] py-3.5 rounded-xl border-slate-200 shadow-sm" : "flex-1 w-full py-3.5 rounded-xl border-slate-200 shadow-sm"}
          />
          {formattedBookingUrl && (
            <a
