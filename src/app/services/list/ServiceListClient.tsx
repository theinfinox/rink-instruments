...
interface Props {
  ...
  startups: { slug: string; name: string }[];
...
export default function ServiceListClient({
  ...
  function buildUrl(overrides: Partial<InitialFilters> & { page?: string }) {
    ...
    if (merged.startup) params.set('startup', merged.startup);
    if (merged.district) params.set('district', merged.district);
    if (merged.certification) params.set('certification', merged.certification);
  ...
  function clearFilters() {
    const empty: InitialFilters = { q: '', category: '', startup: '', district: '', certification: '' };
    setFilters(empty);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          <nav className="flex items-center gap-1.5 text-xs text-text-secondary mb-3">
            ...
            {filters.category && (
              <>
                <ChevronRight className="w-3 h-3 text-slate-300" />
                <span className="text-accent font-semibold">{filters.category}</span>
              </>
            )}
          </nav>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-text-secondary mt-1">
                Showing {total} {total === 1 ? 'service' : 'services'} from Kerala startups.
            ...
            <div className="md:w-64 flex-shrink-0">
              <div className="filter-sidebar sticky top-20">
                ...
                <div className="filter-group">
                  <label htmlFor="filter-category" className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">
                    Category
                  </label>
                  <select
                    id="filter-category"
                  ...
                  >
                    ...
                    {[...categories].sort((a, b) => a.name.localeCompare(b.name)).map(s => (
                ...
                <div className="filter-group">
                  <label htmlFor="filter-startup" className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">
                    Startup
                  </label>
                  <select
                    id="filter-startup"
                    value={filters.startup}
                    onChange={e => applyFilter('startup', e.target.value)}
                    className="w-full text-sm border border-border rounded-lg px-3 py-2 text-text-primary bg-card focus:outline-none focus:border-accent-secondary focus:ring-1 focus:ring-accent-secondary/20"
                  >
                    <option value="">All Startups</option>
                    {[...startups].sort((a, b) => a.name.localeCompare(b.name)).map(inst => (
                ...
                <div className="filter-group">
                  <label htmlFor="filter-certification" className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">
                    Certifications
                  </label>
                  <select
